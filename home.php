...
<body>
	<div class="container">
		<div class="row">
			...
			<div class="column"></div>
			<div class="column"></div>
			<div class="column"><a href="logout.php"><button class="button-clear">Log out</button></a></div>
		</div>
	</div>
		...
		<div class="row">
			<div class="column column-60 column-offset-20">
				<form method="post" enctype="multipart/form-data" action="home.php"  > 
					...
					<span style="color: green;"><?php echo isset($msg)? $msg : "";?></span><br>
					<label for="fuel_capacity">Fuel Capacity(litres):</label>
					...
					<div>
						<form action="" method="POST" enctype="multipart/form-data">
							...
							<p><input type="file" name="file"  value="<?php echo isset($upload_error)? $upload_error : "";?>" onchange="previewFile(this);" required></p>
						</p>
				...
				if($all_user){
					...
					foreach ($all_user as $single_user) {
						...
						<tr>
							...
							<td><?php echo isset($single_user['displacement'])?$single_user['displacement']:""; ?> cc</td>
							<td><?php echo isset($single_user['transmission'])?$single_user['transmission']:""; ?></td>
							<td><?php echo isset($single_user['power'])?$single_user['power']:""; ?> hp</td>
							<td><?php echo isset($single_user['torque'])?$single_user['torque']:""; ?> Nm</td>
							<td><?php echo isset($single_user['fuel_capacity'])?$single_user['fuel_capacity']:""; ?> litres</td>
							...
							<td class="text-right"> 
								...
								<a class="fa fa-fw fa-trash" href="home.php?action=DEL&ID=<?php echo $single_user['id'];?>" onclick="return confirm('Are you sure you want to Remove?');">
								</a>
